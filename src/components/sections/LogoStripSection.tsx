    ...
    <section className="py-20 md:py-28 border-t border-border overflow-hidden">
      ...
      <div className="flex w-max animate-logo-scroll group-hover:[animation-play-state:paused]">
          {allLogos.map((logo, index) => (
            <div key={`${logo.alt}-${index}`} className="flex-shrink-0 flex items-center justify-center px-8 md:px-12 <div key={`${logo.alt}-${index}`} className="flex-shrink-0 flex items-center justify-center px-10 md:px-16 w-[160px] md:w-[220px] h-[60px] md:h-[80px]">">
